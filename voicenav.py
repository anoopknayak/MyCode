...
class VoiceNav:
    def __init__(self):
        ...
        rospy.Subscriber('speech_to_text', String, self.speech_callback)
    ...
    def speech_callback(self, msg):
        ...
        if command is not None:
            ...
            loc = msg.data.lower().find("save this location as")
            if loc > -1:
                data = msg.data[loc:].replace("save this location as ", "", 1)
                cmd = "rosrun speech save_current_location.py %s" % (data)
                call(cmd, shell=True)
                return
